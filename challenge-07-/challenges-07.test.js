...
const forLoopTwoToThe = (arr1) => {
  let result = [];
  for (let index = 0; index < arr1.length; index++) {
    let math = Math.pow(2, arr1[index]);
    result.push(math);
  }
  return result;
};
...
const mapTwoToThe = (arr1) => {
  let newMap = arr1.map(index => Math.pow(2, index));
  return newMap;
};
...
const charCode = (arr1) => {
  ...
  return arr1.map((value, index, array) => {
